...
// ============================================================
const SENSORS = [
  { name: "SSH Honeypot",   port: "2222/tcp", key: 2222, color: "#DC9657" },
  { name: "Telnet Legacy",  port: "2323/tcp", key: 2323, color: "#C24B4B" },
  { name: "HTTP Finance",   port: "8088/tcp", key: 8088, color: "#6e7d53" },
  { name: "HTTPS Ops API",  port: "8443/tcp", key: 8443, color: "#DC9657" },
...
function renderSensors(sessionsArr, status, eventsArr) {
  ...
  grid.innerHTML = SENSORS.map((s) => {
    ...
    const statusColor = !running ? "#8c9680" : isServiceListening ? (engaged ? "#C24B4B" : "#DC9657") : "#C24B4B";
...
function generateExecutiveBriefHtml(sess, events = []) {
  ...
  return `
    ...
    <div style="margin-top:24px;padding-top:12px;border-top:1px solid #FDF5ED;display:flex;justify-content:space-between;align-items:center;font-size:11px;color:var(--text-muted)">
...
function openSessionModal(sess, events = []) {
  ...
  if (statusEl) {
    ...
    statusEl.style.color = sess.contained ? "#9E5F27" : isLive ? "var(--rose)" : "var(--text-secondary)";
  ...
  if (ragList && report.sources?.length) {
    ...
    ragList.innerHTML = report.sources.map(src => `
      <div class="rag-item">
        ...
        ${src.score ? `<span style="font-size:10px;font-weight:700;padding:1px 6px;border-radius:2px;background:var(--primary-light);color:#9E5F27">${typeof src.score === "number" ? src.score.toFixed(2) : esc(src.score)}</span>` : ""}
      </div>`).join("");
  }
...
function updateStatusUI(status) {
  ...
  if (pauseBtn) {
    ...
    if (pauseIcon) {
      ...
      pauseIcon.style.color = running ? "#7d9cb7" : "#9E5F27";
    }
    if (pauseLabel) {
  ...
  if (modelBadge) {
    ...
    if (!running) {
      ...
      modelBadge.style.color = "#9E5F27";
      modelBadge.style.borderColor = "#DC9657";
    } else {
...
function initLeafletMap() {
  ...
  try {
    ...
    const socIcon = L.divIcon({
      html: `<div style="background:#DC9657;color:#fff;font-family:var(--font-mono);font-size:9px;font-weight:700;padding:2px 7px;border-radius:4px;border:1.5px solid #fff;box-shadow:0 0 10px rgba(220,150,87,0.6);white-space:nowrap">🛡️ VALENS GRID</div>`,
  ...
  $("dossier-btn-lure")?.addEventListener("click", () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (notesEl) {
          notesEl.innerHTML = `<span style="color:#EA580C;font-weight:700">🎯 Precision Honey-Lure Triangulation Active:</span> Pinpointed to physical subscriber node at <strong>${lat.toFixed(5)}, ${lon.toFixed(5)}</strong> (±${acc}m accuracy). Demonstrates client-side sensor honeypot trap vs standard BGP ISP network routing.`;
        }
        ...
        if (leafletMap && leafletMarkersLayer) {
          const lureIcon = L.divIcon({
            html: `<div class="leaflet-latest-marker" style="cursor:pointer">
                     ...
                     <span class="marker-dot" style="background:#FF6F43"></span>
                     <span class="marker-badge" style="background:#EA580C">🎯 PHYSICAL NODE: ${lat.toFixed(4)}, ${lon.toFixed(4)}</span>
                   </div>`,
            className: "leaflet-custom-marker-wrap",
            iconSize: [195, 26],
            iconAnchor: [8, 11]
          ...
          lureMarker.bindPopup(`
            <div style="font-family:Inter,sans-serif;min-width:220px">
              <div style="font-weight:700;font-size:13px;color:#EA580C;display:flex;align-items:center;gap:6px">
                <span>🎯</span>
  ...
  $("btn-commit-ir")?.addEventListener("click", async () => {
    ...
    if (term && termLines) {
      ...
      const logItems = [
        ...
        `<span style="color:#64748b;">[${now}]</span> <span style="color:#DC9657;">✓ [CONTAINMENT]</span> Disconnected socket & isolated session <code>${sessShort}</code> into synthetic sandbox.`,
      ];
      if (shouldBlockIP) {
        logItems.push(
      ...
      logItems.push(
        ...
        `<span style="color:#64748b;">[${now}]</span> <span style="color:#DC9657;font-weight:700;">★ ENFORCEMENT SUMMARY:</span> ${checked.length} action(s) active in live perimeter.`
      );
  ...
  if (data.latest_event) {
    ...
    if (badge) {
      ...
      if (ev.blocked) {
        ...
        badge.style.background = "#FDF5ED";
        badge.style.color = "#9E5F27";
      }
    }
  }
  ...
  if (data.latest_events && data.latest_events.length > 0) {
    ...
    if (feed && list) {
      ...
      list.innerHTML = data.latest_events.slice(0, 5).map(e => {
        const bg = e.blocked ? "rgba(239, 68, 68, 0.08)" : "rgba(220, 150, 87, 0.05)";
        const border = e.blocked ? "rgba(239, 68, 68, 0.2)" : "rgba(220, 150, 87, 0.15)";
        const statusBadge = e.blocked 
          ...
          : `<span style="color:#DC9657;font-weight:700;font-size:10px;background:rgba(220,150,87,0.15);padding:1px 5px;border-radius:3px;">${e.status_code || 200} FORWARDED</span>`;
  ...
  try {
    ...
    if (owaspList && data.owasp_breakdown) {
      owaspList.innerHTML = Object.entries(data.owasp_breakdown).map(([cat, score]) => `
        <div style="display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,0.03);padding:8px 12px;border-radius:6px;border:1px solid rgba(255,255,255,0.05);font-size:11px;">
          <span>${cat}</span>
          <span style="font-family:var(--font-mono);font-weight:700;color:${score >= 90 ? '#DC9657' : '#f59e0b'}">${score}% Pass</span>
        </div>
      `).join("");
    }
    ...
    if (patchList && data.virtual_patches_applied) {
      patchList.innerHTML = data.virtual_patches_applied.map(p => `
        <span style="font-size:10px;font-family:var(--font-mono);background:rgba(220,150,87,0.15);color:#DC9657;border:1px solid rgba(220,150,87,0.3);padding:3px 8px;border-radius:4px;font-weight:700;">✓ ${p}</span>
      `).join("");
    }
    ...
    if (ips.length === 0) {
      container.innerHTML = `
        <div style="text-align:center;padding:24px 16px;background:rgba(255,255,255,0.02);border-radius:8px;border:1px dashed rgba(255,255,255,0.1);color:var(--text-muted);font-size:12px;">
          <span class="material-symbols-outlined" style="font-size:28px;color:#DC9657;display:block;margin-bottom:6px;">check_circle</span>
