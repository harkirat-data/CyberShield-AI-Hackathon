...
# from rag.core.pipeline import RAGPipeline (lazy imported below)
...
class Orchestrator:
    def __init__(self, use_rag: bool = True, use_llm: bool = True):
        ...
        if use_rag or use_llm:
            try:
                from rag.core.pipeline import RAGPipeline
                self.pipeline = RAGPipeline()
            except Exception as e:
                print(f"[orchestrator] RAGPipeline init failed: {e}")
                self.pipeline = None
        else:
            self.pipeline = None
...
def create_app():
    ...
    app = FastAPI(title="VALENS Orchestrator", version="1.0")
